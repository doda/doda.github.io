(function(){const a=33;let o=!1,i=!0,t=new Set;const d=`
  ...
  `,p=`
    ...
    void main() {
      ...
      // Dusty palette - subtle warm/cool interplay
      vec3 lightPageBg = vec3(1.0, 1.0, 1.0);
      vec3 lightLayer1 = vec3(0.91, 0.89, 0.87);  // Warm sand
      vec3 lightLayer2 = vec3(0.84, 0.86, 0.88);  // Cool slate
      vec3 lightLayer3 = vec3(0.78, 0.80, 0.82);  // Steel blue-gray

      vec3 darkPageBg = vec3(0.11, 0.12, 0.14);
      vec3 darkLayer1 = vec3(0.20, 0.18, 0.17);   // Warm charcoal
      vec3 darkLayer2 = vec3(0.22, 0.24, 0.27);   // Cool slate
      vec3 darkLayer3 = vec3(0.28, 0.30, 0.34);   // Steel blue
      ...
      // Per-blob lightness variation
      float varStrength = 0.06;
      float var1 = 1.0 + (hue1 - 0.5) * varStrength;
      float var2 = 1.0 + (hue2 - 0.5) * varStrength;
      float var3 = 1.0 + (hue3 - 0.5) * varStrength;

      // Compose layers back to front with per-blob variation
      vec3 color = mix(pageBg, layer1 * var1, blend1);
      color = mix(color, layer2 * var2, blend2);
      color = mix(color, layer3 * var3, blend3);
