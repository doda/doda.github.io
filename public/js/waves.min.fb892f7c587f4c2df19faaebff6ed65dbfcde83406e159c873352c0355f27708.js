(function(){const a=33;let o=!1,i=!0,t=new Set;const d=`
  ...
  `,p=`
    ...
    float metaball(vec2 p, float time) {
      ...
      for (int y = -1; y <= 1; y++) {
        for (int x = -1; x <= 1; x++) {
          ...
          sum += influence * influence * influence * influence;
        }
      }
      return sum;
    }
    ...
    vec2 warpCoords(vec2 p, float time) {
      float warp1 = vnoise(p * 0.15 + time * 0.05);
      float warp2 = vnoise(p * 0.1 - time * 0.03 + 100.0);
      return p + vec2(warp1, warp2) * 0.1;
    }
    ...
    void main() {
      ...
      vec2 p1 = p * 0.15 + vec2(u_time * 0.001, u_time * 0.0007);
      float field1 = metaball(warpCoords(p1, u_time * 0.03), u_time * 0.03) + wallInfluence;

      // Layer 2 - medium
      vec2 p2 = p * 0.3 + vec2(u_time * 0.0015, -u_time * 0.001);
      float field2 = metaball(warpCoords(p2 + 50.0, u_time * 0.05), u_time * 0.05) + wallInfluence;

      // Layer 3 - smallest, fastest (front)
      vec2 p3 = p * 0.5 + vec2(u_time * 0.002, u_time * 0.001);
      float field3 = metaball(warpCoords(p3 + 100.0, u_time * 0.07), u_time * 0.07) + wallInfluence;
      ...
      float blend1 = smoothstep(0.6 - aaWidth, 0.6 + aaWidth, field1);
      float blend2 = smoothstep(0.7 - aaWidth, 0.7 + aaWidth, field2);
      float blend3 = smoothstep(0.8 - aaWidth, 0.8 + aaWidth, field3);
