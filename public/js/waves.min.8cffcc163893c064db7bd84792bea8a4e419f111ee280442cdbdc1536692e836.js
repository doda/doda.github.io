(function(){const a=33;let o=!1,i=!0,t=new Set;const d=`
  ...
  `,p=`
    ...
    void main() {
      ...
      // Monochrome palette - wide range
      vec3 lightPageBg = vec3(1.0, 1.0, 1.0);
      vec3 lightLayer1 = vec3(0.95, 0.95, 0.95);  // Lightest
      vec3 lightLayer2 = vec3(0.80, 0.80, 0.80);  // Medium
      vec3 lightLayer3 = vec3(0.55, 0.55, 0.55);  // Darkest

      vec3 darkPageBg = vec3(0.10, 0.10, 0.10);
      vec3 darkLayer1 = vec3(0.15, 0.15, 0.15);   // Lightest
      vec3 darkLayer2 = vec3(0.30, 0.30, 0.30);   // Medium
      vec3 darkLayer3 = vec3(0.50, 0.50, 0.50);   // Darkest
      ...
      // Undulating waves
      boundary += sin(xCoord * 0.008 + u_time * 0.15) * 25.0 * u_dpr;         // Primary wave
      boundary += sin(xCoord * 0.015 - u_time * 0.1) * 15.0 * u_dpr;          // Secondary wave (opposite dir)
      boundary += sin(xCoord * 0.025 + u_time * 0.2) * 8.0 * u_dpr;           // Tertiary ripple
      // Random variation on top
      boundary += vnoise(vec2(xCoord * 0.01, u_time * 0.03)) * 12.0 * u_dpr;
