(function(){const a=33;let o=!1,i=!0,t=new Set;const d=`
  ...
  `,p=`
    ...
    // Voronoi - returns distance to nearest cell edge
    float voronoi(vec2 p, float time) {
      ...
      // Return edge distance (difference between closest and second closest)
    ...
    void main() {
      ...
      // Layer 1 - largest cells, slowest (back)
      vec2 p1 = p * 0.4 + vec2(u_time * 0.015, u_time * 0.01);
      float edge1 = voronoi(warpCoords(p1, u_time * 0.4), u_time * 0.4);

      // Layer 2 - medium cells
      vec2 p2 = p * 0.6 + vec2(u_time * 0.025, -u_time * 0.015);
      float edge2 = voronoi(warpCoords(p2 + 50.0, u_time * 0.6), u_time * 0.6);

      // Layer 3 - smallest cells, fastest (front)
      vec2 p3 = p * 0.9 + vec2(u_time * 0.04, u_time * 0.02);
      float edge3 = voronoi(warpCoords(p3 + 100.0, u_time * 0.8), u_time * 0.8);

      float taperRange = 30.0 * u_dpr;
      float taper = smoothstep(-taperRange, 0.0, distToBoundary);

      // Invert and apply wall influence (cells fill from boundary)
      float field1 = (1.0 - edge1 * 2.5) * taper + wallInfluence;
      float field2 = (1.0 - edge2 * 2.5) * taper + wallInfluence;
      float field3 = (1.0 - edge3 * 2.5) * taper + wallInfluence;

      float aaWidth = 0.02;
      float blend1 = smoothstep(0.4 - aaWidth, 0.4 + aaWidth, field1);
      float blend2 = smoothstep(0.5 - aaWidth, 0.5 + aaWidth, field2);
      float blend3 = smoothstep(0.6 - aaWidth, 0.6 + aaWidth, field3);

      // Compose layers back to front
      vec3 color = mix(pageBg, layer1, blend1);
      color = mix(color, layer2, blend2);
      color = mix(color, layer3, blend3);
