(function(){const a=33;let o=!1,i=!0,t=new Set;const d=`
  ...
  `,p=`
    ...
    void main() {
      ...
      boundary += vnoise(vec2(xCoord * 0.003, 0.0)) * 20.0 * u_dpr;           // Large random bumps (static)
      boundary += vnoise(vec2(xCoord * 0.015, 100.0)) * 12.0 * u_dpr;         // Medium variation (static)
      boundary += vnoise(vec2(xCoord * 0.05, 200.0)) * 5.0 * u_dpr;           // Small jagged detail (static)
      boundary += vnoise(vec2(xCoord * 0.008, u_time * 0.02)) * 4.0 * u_dpr;  // Subtle slow movement
